...
```
Так же `platform_id` было заменено на `standard-v3` по примерам документации Терраформ ресурса Яндекс
```hcl
platform_id = "standard-v3"
```
Также в параметре `standart-v3`в исходном коде была синтаксическая ошибка - правильно - `standard-v3`
---
...
## Шаг 2. Переменная первой VM

Позволил себе сразу все добавить в массив для удобства работы в будущем. Параметры первой виртуальной машины были объединены в переменную.
...
Для получения информации о созданных виртуальных машинах был модифицирован файл:
...
Переменные, которые больше не используются после объединения параметров в `metadata`, были закомментированы/удалены.
